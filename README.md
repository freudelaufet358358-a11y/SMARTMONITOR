...
| 大学の時間割 (UNIPA) | ○ | **UNIPA は ICS 出力なし** → `config.json` の `timetable.slots` に手書き → コマ式の表で表示（今日列・開講中コマを強調） |
...
dashboard/                    スタンドアロンダッシュボード本体（採用中）
  ...
  config.json                 時間割（コマ式 slots）・RSS・天気（編集して使う）
serve/
...
4. （参考）[`docs/02-home-assistant.md`](docs/02-home-assistant.md) / [`docs/04-dashboard.md`](docs/04-dashboard.md) — HA 版（不採用だが残置）
