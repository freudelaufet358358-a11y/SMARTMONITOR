...
def run_fetch():
    """設定変更後に天気/RSS を取り直し、version.txt を更新して全画面を再読込させる。

    （応答をブロックしないよう別スレッドで呼ぶ。version.txt の更新は app.js が
    検知して location.reload() するため、Webhook 自動更新と同じ仕組みで反映される）
    """
    ...
    # データ取得が終わってから version.txt を更新 → 各画面が新データで再読込
    try:
        with open(os.path.join(WWW, "version.txt"), "w", encoding="utf-8") as f:
            f.write(str(int(time.time())))
    except Exception as e:  # noqa: BLE001
        print(f"version bump failed: {e}", file=sys.stderr)
